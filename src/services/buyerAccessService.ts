...
export const grantBuyerAccess = async (
  ...
  try {
    ...
    await addGroupMember(novaGroupId, buyerAccountId);
...
export const revokeBuyerAccess = async (
  ...
  try {
    ...
    await revokeGroupMember(novaGroupId, buyerAccountId);
...
export const grantAccessToAllPendingBuyers = async (
  ...
  for (let i = 0; i < pendingBuyers.length; i++) {
    ...
    try {
      await grantBuyerAccess(productId, novaGroupId, buyer, callFunction);
      success.push(buyer);
    } catch (error) {
